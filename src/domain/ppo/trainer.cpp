...
PPOTrainer::PPOTrainer(config::TrainerConfig config, env::EnvironmentPack environment_pack)
    ...
    environment_name_ = environment_pack.display_name;
    observation_dim_ = environment_pack.observation_dim;
    action_dim_ = environment_pack.action_dim;

    model_ = PolicyValueModel(observation_dim_, action_dim_, config_.hidden_dim);
    model_->to(device_);
    optimizer_ = std::make_unique<torch::optim::Adam>(
        model_->parameters(),
        torch::optim::AdamOptions(config_.ppo.learning_rate)
    );
    ...
    parameter_count_k_ = static_cast<float>(model_->parameter_count()) / 1000.0f;
...
std::vector<TrainingMetrics> PPOTrainer::train() {
    ...
    for (int64_t update = 1; update <= config_.total_updates; ++update) {
        ...
        const auto sample_count = static_cast<float>(config_.num_envs * config_.ppo.rollout_steps);
        ...
        metric.completed_episodes = static_cast<int64_t>(finished_returns_.size());
...
std::vector<LiveStep> PPOTrainer::run_live_episode(const int64_t max_steps, const bool deterministic, std::ostream* stream) {
    ...
    for (int64_t step_index = 0; step_index < max_steps; ++step_index) {
        ...
        const auto policy = model_->act(batch_observation, deterministic);
...
std::vector<EpisodeRecord> PPOTrainer::consume_completed_episodes() {
    std::vector<EpisodeRecord> out;
    out.swap(pending_episode_records_);
    return out;
}

PolicyValueModel& PPOTrainer::model() {
    return model_;
}

const std::string& PPOTrainer::environment_name() const {
    return environment_name_;
}

int64_t PPOTrainer::observation_dim() const {
    return observation_dim_;
}

int64_t PPOTrainer::action_dim() const {
    return action_dim_;
}
...
RolloutBatch PPOTrainer::collect_rollout() {
    ...
    for (int64_t step = 0; step < config_.ppo.rollout_steps; ++step) {
        ...
        for (int64_t env_index = 0; env_index < step_env_count; ++env_index) {
            ...
            if (done) {
                const float success = environments_[static_cast<std::size_t>(env_index)]->success_signal(result);
                const auto episode_return = episode_returns_[static_cast<std::size_t>(env_index)];
                const auto episode_length = episode_lengths_[static_cast<std::size_t>(env_index)];

                finished_returns_.push_back(episode_return);
                finished_lengths_.push_back(episode_length);
                finished_successes_.push_back(success);
                pending_episode_records_.push_back(
                    {
                        next_episode_index_++,
                        next_total_steps,
                        episode_return,
...
TrainingMetrics PPOTrainer::update_policy(const RolloutBatch& batch, const int64_t update_index) {
    ...
    const float entropy_weight = config_.ppo.entropy_weight * (1.15f - 0.35f * progress);
    ...
    for (int64_t epoch = 0; epoch < config_.ppo.ppo_epochs; ++epoch) {
        if (stop_early) {
            break;
        }
        ...
        for (int64_t start = 0; start < sample_count; start += config_.ppo.minibatch_size) {
            ...
            const auto [new_log_probs, entropy] = model_->evaluate_actions(obs, actions);
            const auto values = model_->values(obs);
            const auto std = model_->policy_std(obs);
            const auto log_ratio = new_log_probs - old_log_probs;
            const auto ratio = torch::exp(log_ratio);
            const auto clipped_ratio =
                torch::clamp(ratio, 1.0f - config_.ppo.clip_epsilon, 1.0f + config_.ppo.clip_epsilon);
            ...
            const auto clipped_values = normalized_old_values + torch::clamp(
                ...
                -config_.ppo.value_clip_epsilon,
                config_.ppo.value_clip_epsilon
            );
            ...
            const auto std_floor_penalty = torch::relu(config_.ppo.target_action_std - std.mean());
            const auto total_loss =
                policy_loss +
                config_.ppo.value_loss_weight * critic_loss -
            ...
            last_value_loss = critic_loss.item<float>();
            last_entropy = entropy_bonus.item<float>();
            last_approx_kl = ((ratio - 1.0f) - log_ratio).mean().item<float>();
            last_clip_fraction =
                ((ratio < (1.0f - config_.ppo.clip_epsilon)) | (ratio > (1.0f + config_.ppo.clip_epsilon)))
                    .to(torch::kFloat32)
                    .mean()
                    .item<float>();
    ...
    return {
        ...
        0.0f,
        parameter_count_k_,
        static_cast<int64_t>(finished_returns_.size())
    };
}
