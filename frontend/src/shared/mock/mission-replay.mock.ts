...
function createDataset(runId: string, label: string, phase: number): ReplayRunDataset {
  ...
  const run: RunSummary = {
    ...
    backend: "libtorch_cpu",
    torchDevice: "cpu",
    computeBackendRequested: "cpu",
    computeBackendResolved: "cpu",
    cudaFallbackUsed: false,
    avgInferenceLatencyMs: 0.031,
    p50InferenceLatencyMs: 0.029,
    p95InferenceLatencyMs: 0.041,
    deterministic: true,
